<?php
	function mobile_gateway_icon( $gateways ) {
		if ( isset( $gateways['mobile'] ) ) {
	...
	add_filter( 'woocommerce_available_payment_gateways', 'mobile_gateway_icon' );

add_action('plugins_loaded', 'woocommerce_mobile_payu_init', 0);
function woocommerce_mobile_payu_init(){
  if(!class_exists('WC_Payment_Gateway')) return;
 
  class WC_mobile_Payu extends WC_Payment_Gateway{
    public function __construct(){
      $this -> id = 'mobile';
      $this -> method_title = 'Со счета мобильного телефона'; 
   ...
   //   add_action('init', array(&$this, 'check_payu_response'));
      ...
      add_action('woocommerce_receipt_mobile', array(&$this, 'receipt_page'));
   }
    function init_form_fields(){
		...
		'enabled' => array(
			...
			'type' => 'checkbox',
			'label' => __('Включить оплату со счета мобильного телефона','yandex_money'),
			'default' => 'no'),
		'title' => array(
			'title' => __('Заголовок','yandex_money'),
			'type'=> 'text',
			'description' => __('Название, которое пользователь видит во время оплаты','yandex_money'),
			'default' => __('Со счета мобильного телефона','yandex_money')),
		'description' => array(
			'title' => __('Описание','yandex_money'),
			'type' => 'textarea',
			'description' => __('Описание, которое пользователь видит во время оплаты','yandex_money'),
			'default' => __('Оплата со счета мобильного телефона','yandex_money'))
		);
		
    }
 
       public function admin_options(){
		echo '<h3>'.__('Платеж со счета мобильного телефона','yandex_money').'</h3>';
    ...
    public function generate_payu_form($order_id){
	    ...
	    $result ='';
		$result .= '<form name=ShopForm method="POST" id="submit_mobile_payment_form" action="'.$sendurl.'">';
			...
			$result .= '<input name="paymentType" value="MC" type="hidden">';
		...
		';
		$result .='jQuery(document).ready(function ($){ jQuery("#submit_mobile_payment_form").submit(); });';
		$result .='</script>';
		$result .='</form>';
   ...
   /**
     ...
     **/
    function woocommerce_add_mobile_payu_gateway($methods) {
        $methods[] = 'WC_mobile_Payu';
        return $methods;
    }
 
    add_filter('woocommerce_payment_gateways', 'woocommerce_add_mobile_payu_gateway' );
}
