<?php
	function sberbank_gateway_icon( $gateways ) {
		if ( isset( $gateways['sberbank'] ) ) {
	...
	add_filter( 'woocommerce_available_payment_gateways', 'sberbank_gateway_icon' );

add_action('plugins_loaded', 'woocommerce_sberbank_payu_init', 0);
function woocommerce_sberbank_payu_init(){
  if(!class_exists('WC_Payment_Gateway')) return;
 
  class WC_sberbank_Payu extends WC_Payment_Gateway{
   ...
   //   add_action('init', array(&$this, 'check_payu_response'));
      ...
      add_action('woocommerce_receipt_sberbank', array(&$this, 'receipt_page'));
   }
    function init_form_fields(){
		...
		'enabled' => array(
			...
			'type' => 'checkbox',
			'label' => __('Включить оплату через Сбербанк: оплата по SMS или Сбербанк Онлайн','yandex_money'),
			'default' => 'no'),
		'title' => array(
			'title' => __('Заголовок','yandex_money'),
			'type'=> 'text',
			'description' => __('Название, которое пользователь видит во время оплаты','yandex_money'),
			'default' => __('Сбербанк: оплата по SMS или Сбербанк Онлайн','yandex_money')),
		'description' => array(
			'title' => __('Описание','yandex_money'),
			'type' => 'textarea',
			'description' => __('Описание, которое пользователь видит во время оплаты','yandex_money'),
			'default' => __('Оплата через Сбербанк: оплата по SMS или Сбербанк Онлайн','yandex_money'))
		);
		
    }
 
       public function admin_options(){
		echo '<h3>'.__('Оплата через Сбербанк: оплата по SMS или Сбербанк Онлайн','yandex_money').'</h3>';
    ...
    public function generate_payu_form($order_id){
	    ...
	    $result ='';
		$result .= '<form name=ShopForm method="POST" id="submit_sberbank_payment_form" action="'.$sendurl.'">';
			...
			$result .= '<input name="paymentType" value="SB" type="hidden">';
		...
		';
		$result .='jQuery(document).ready(function ($){ jQuery("#submit_sberbank_payment_form").submit(); });';
		$result .='</script>';
		$result .='</form>';
   ...
   /**
     ...
     **/
    function woocommerce_add_sberbank_payu_gateway($methods) {
        $methods[] = 'WC_sberbank_Payu';
        return $methods;
    }
 
    add_filter('woocommerce_payment_gateways', 'woocommerce_add_sberbank_payu_gateway' );
}
