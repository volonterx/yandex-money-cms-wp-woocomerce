<?php
	function masterpass_gateway_icon( $gateways ) {
		if ( isset( $gateways['masterpass'] ) ) {
	...
	add_filter( 'woocommerce_available_payment_gateways', 'masterpass_gateway_icon' );

add_action('plugins_loaded', 'woocommerce_masterpass_payu_init', 0);
function woocommerce_masterpass_payu_init(){
  if(!class_exists('WC_Payment_Gateway')) return;
 
  class WC_masterpass_Payu extends WC_Payment_Gateway{
   ...
   //   add_action('init', array(&$this, 'check_payu_response'));
    ...
    public function generate_payu_form($order_id){
	    ...
	    $result ='';
		$result .= '<form name=ShopForm method="POST" id="submit_masterpass_payment_form" action="'.$sendurl.'">';
			...
			$result .= '<input name="paymentType" value="MA" type="hidden">';
		...
		';
		$result .='jQuery(document).ready(function ($){ jQuery("#submit_masterpass_payment_form").submit(); });';
		$result .='</script>';
		$result .='</form>';
   ...
   /**
     ...
     **/
    function woocommerce_add_masterpass_payu_gateway($methods) {
        $methods[] = 'WC_masterpass_Payu';
        return $methods;
    }
 
    add_filter('woocommerce_payment_gateways', 'woocommerce_add_masterpass_payu_gateway' );
}
